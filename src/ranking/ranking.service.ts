  ...
  async getGeneralRanking() {
    const result = await this.db.query(
      ...
      SELECT
        ...
        (SELECT COALESCE(SUM(points), 0)::INT FROM bracket_predictions WHERE user_id = u.id AND match_id IS NULL) AS bracket_points,
        (
          ...
          (SELECT COALESCE(SUM(points), 0) FROM bracket_predictions WHERE user_id = u.id AND match_id IS NULL)
        )::INT AS total_points
      FROM users u
